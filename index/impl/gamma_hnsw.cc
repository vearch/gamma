...
GammaHNSW::GammaHNSW(int M) : HNSW(M) { nlinks = M; }

GammaHNSW::GammaHNSW() : HNSW(32) { nlinks = 32; }
...
void add_link(GammaHNSW& hnsw, DistanceComputer& qdis, storage_idx_t src,
...
void search_neighbors_to_add(GammaHNSW& hnsw, DistanceComputer& qdis,
...
void greedy_update_nearest(const GammaHNSW& hnsw, DistanceComputer& qdis,
...
void GammaHNSW::AddLinksStartingFrom(DistanceComputer& ptdis,
                                     storage_idx_t pt_id, storage_idx_t nearest,
...
int GammaHNSW::AddWithLocks(DistanceComputer& ptdis, int pt_level, int pt_id,
...
#pragma omp critical
  ...
  for (; level >= 0; level--) {
    AddLinksStartingFrom(ptdis, pt_id, nearest, d_nearest, level, locks.data());
  }
...
int GammaHNSW::SearchFromCandidates(DistanceComputer& qdis, int k, idx_t* I,
                                    float* D, MinimaxHeap& candidates, int level,
                                    int efSearch, 
...
std::priority_queue<Node> GammaHNSW::SearchFromCandidateUnbounded(
  ...
  if (retrieval_context->IsValid(node.second) && 
      retrieval_context->IsSimilarScoreValid(abs(node.first))) {
...
void GammaHNSW::Search(DistanceComputer& qdis, int k, idx_t* I, float* D,
                       int efSearch, 
                       const RetrievalContext *retrieval_context) const {
  if (entry_point == -1) {
  ...
  if (search_bounded_queue) {
    ...
    SearchFromCandidates(qdis, k, I, D, candidates, 0, efSearch, retrieval_context);
  } else {
    std::priority_queue<Node> top_candidates = SearchFromCandidateUnbounded(
        Node(d_nearest, nearest), qdis, ef, retrieval_context);
